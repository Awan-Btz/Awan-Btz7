...
async function starts() {
	...
	client.on('qr', () => {
		client.sendMessage("62895411143170@s.whatsapp.net", `Bot Sudah Sudah Aktif`, MessageType.extendedText)
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Scan QRnya Cepet'))
	})

	fs.existsSync('./sean.json') && client.loadAuthInfo('./sean.json')
	client.on('connecting', () => {
		start('2', 'Menyambungkan')
	})
	client.on('open', () => {
		success('2', 'Sukses, Sc by sean ')
	})
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./sean.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
...
let settingstatus = 0;
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await client.setStatus(`Aktif selama ${uptime} | spam/vc/telfon?block | Sewa Bot Only🗿`).catch((_) => _);
      settingstatus = new Date() * 1;
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'sean'; if (!author) author = 'Bot';	
...
`
				...
				case 'help':
					...
					ptod = "62895411143170@s.whatsapp.net"
...
Semoga harimu Menyenangkan 
 ...
 ╭◪ INFO CREATOR
├❍ *chat owner wasap : wa.me/62895411143170*
├❖ *TAG : @${ptod.split("@")[0]}*
╰─────────────────────╯
...
var imgs = await client.prepareMessage('0@c.us', thumb, image, { thumbnail: thumb })
            ...
            var ctlg = await client.prepareMessageFromContent(from, {
                "productMessage": {
                    ...
                    "businessOwnerJid": "62895411143170@s.whatsapp.net",
                }
            }, { quoted: mek, mimetype: 'image/jpeg' })
            client.relayWAMessage(ctlg)
            break
                case 'bebantot':
                reply(`*SEMOGA SALING MENGHARGAI YA*`)
                break
                case 'donasi':
				case 'donate':
...
titid = 'butuh info tentang apa ya?'
          ...
          },
          {
            ...
            + `FN: SeanChan\n`  
            + `ORG: BOT INDO;\n` 
            + `TEL;type=CELL;type=VOICE;waid=62895411143170:+62895411143170\n`  
            + 'END:VCARD'  
  client.sendMessage(from, {displayname: "Jeff", vcard: vcard2}, MessageType.contact, { quoted: mek})
tod = await getBuffer(`https://telegra.ph/file/a2d60fe66e9bc58758af2.jpg`)
                 client.sendMessage(from, tod, image, { quoted: mek, caption: '*JANGAN LUPA SOPAN SANTUN ANDA SANTUN SAYA RAMAH*'})
                 break
                 case 'script':
                case 'sc':
                reply(`*SCRIPT BOT BISA CHAT OWNER*`)
                break
   case 'bokep':
                if (args.length < 1) return reply('*Hayolo belom tobat🤣* *tobat bodoh dosa elu udah banyak itu*')
                break
case 'jadibot':
                if (args.length < 1) return reply('*Fitur Telah Di Hapus Oleh Owner Dikarenakan Banyak Yang spam Fitur Itu Kalo Mau jadi Bot ya run sendiri, bisa chat owner caranya*')
                break
				case 'unban':
					if (!isOwner) return reply(mess.only.ownerB)
					bnnd = body.slice(8)
		...
		case 'd':
			...
			break
                ...
                case 'request':
					...
					client.sendMessage('62895411143170@s.whatsapp.net', options, text, {quoted: mek})
